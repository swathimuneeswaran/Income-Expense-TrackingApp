...
const Profile = () => {
  ...
  const handleDelete = async (incomeId) => {
    try {
      await Axios.delete(`/api/manager/delete-income/${incomeId}`);
      setIncomes((prevIncomes) => prevIncomes.filter(income => income._id !== incomeId));
      setTotalIncome((prevTotal) => prevTotal - incomes.find(income => income._id === incomeId).amount);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleDeleteExp = async (expenseId) => {
    try {
      await Axios.delete(`/api/manager/delete-expense/${expenseId}`);
      setExpense((prevExpense) => prevExpense.filter(expense => expense._id !== expenseId));
      setTotalExp((prevTotal) => prevTotal - expense.find(expense => expense._id === expenseId).amount);
    } catch (error) {
      console.log(error);
    }
  };
    ...
    datasets: [
      {
        ...
        backgroundColor: ["rgb(153, 50, 204)", "rgba(45, 85,255)"],
        borderColor: [" rgba(153,50,204,10)", " rgba(45, 85, 255,10)"],
        borderWidth: 1,
  ...
  return (
    <>
          ...
          <div className="collapse navbar-collapse" id="navbarSupportedContent">
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item mb-2">
                <img
                  src="https://lordicon.com/icons/wired/flat/291-coin-dollar.gif"
      ...
      <div className="pro">
        ...
        <div
          className="col-12 col-lg-5 col-sm-8 col-md-6 box1"
          style={{ display: "flex", gap: "30px", marginTop: "0px" }}
        >
          <div>
            <section
              className="bg-dark van"
              style={{
                color: "white",
                width: "300px",
                height: "130px",
          ...
          <div className="tab2">
            <h3
              style={{
                ...
                fontStyle: "italic",
                fontFamily: "cursive",
                textDecoration: "1px overline white",
                ...
                {incomes.map((income, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <FontAwesomeIcon
                        icon={faIndianRupeeSign}
                        style={{ fontSize: "14px", color: "yellow" }}
                      />
                      {income.amount}
                    </td>
                    <td>
